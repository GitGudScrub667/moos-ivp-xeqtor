#!/bin/bash
#------------------------------------------------------------
#  Script: launch_vehicle.sh   (skara_N_boats - one boat from the pool)
#  NOTE: the ring slot is no longer per-boat here -- it's a placeholder
#  (see init_field.sh); the real slot is pushed live by pArrivalSync on
#  DEPLOY (dynamic roster). Home/RETURN is still looked up by name.
#------------------------------------------------------------
